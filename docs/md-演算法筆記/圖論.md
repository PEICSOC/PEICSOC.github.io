...
void bfs(int ff){
    ...
    q.push(ff);
    visit[ff]=true;             //visit (1/3)
    
    while(!q.empty()){
        int f=q.front();q.pop();
        for(int i:v[f]){ 
            if(!visit[i]){      //visit(2/3)
                visit[i]=true;  // visit(3/3)
                q.push(i);
                cout<<i;
            }
        }
...
9 3 10 11 12 13 14");
    ...
    while(1){
        ...
        int xx=-1,yy=-1;
        for(int i=0;i<4;i++){
            if(vv(x+ton[i],y+tom[i])<mi && !visit[x+ton[i]][y+tom[i]]){
                xx=x+ton[i];
                yy=y+tom[i];
...
int main(){
    ...
    while(!pq.empty()){
        ...
        for(auto i:v[f.second]){
            visit[f.second]=true;
            if(!visit[i.v] && d[f.second]+i.p<d[i.v]){
