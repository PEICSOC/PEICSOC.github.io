...
> 對於這種換錢問題，想要證明使用較小代幣換錢是最好的方法，    
> 該如何證明？假設 \(v[n]\) 為每種代幣（由小到大），\(v[i-1] \times 2 \leq v[i]\) 即可。    
>     
...
> 遇到這個問題我們當然要找個方法排配對誰和誰打誰。    
> 最直覺的方法是，從最小的開始找出比較小的當作對手。    
>     
> 但為何這個方法正確呢？    
>     
> 我們假設 \(a < b\)、\(y < z\)；且 \(b < z\)。    
> 假設「最佳解中有一個解是 \(a\) 與 \(b\) 配對」。    
>     
> 若 \(a\) 與 \(y\) 交換，根據上述關係我們可以知道 \(a < z\)（因為 \(a < b\)、\(b < z\)），    
> 但無法得知 \(y < b\)，所以交換後無法增加勝場數，甚至可能減少。
...
> 前三項合併後為：      
> \(3a + 2b + c\)      
> 由此可知 \(a\) 的影響最大，\(a\) 必須為最小者才能夠達成最小時間總和。      
>       
> 2. 或是我們使用「交換前與交換後的關係」去看      
> 有 \(a, b\) 兩數字      
> 兩數交換前時間總和：\(a + a + b\)      
> 兩數交換後時間總和：\(b + b + a\)      
> 我們希望交換前小於交換後：      
...
> \(\Rightarrow a < b\)      
> 所以比較小的數應該在前面。      
> 下面程式碼使用的方法是將總時間換成 \(3a + 2b + c\) 形式進行運算。
...
> 有兩種情況：    
> 1. \(A, a\) 先：    
> 總額為 \(Aa + (A + B)b = Aa + Ab + Bb\)    
> 2. \(B, b\) 先（交換後）：    
> 總額為 \(Bb + (A + B)a = Bb + Aa + Ba\)    
>     
> 整理過後分別為：    
> \(\text{ans1} = Aa + Ab + Bb\)    
> \(\text{ans2} = Aa + Ba + Bb\)    
>     
> 我們現在想要看怎樣能夠得到比較小。    
> 假設我們希望交換前小於交換後，也就是 \(\text{ans1} < \text{ans2}\)，得到：    
> \[
> Aa + Ab + Bb < Bb + Aa + Ba
> \]    
> \(\Rightarrow Ab + Bb < Bb + Ba\)    
> \(\Rightarrow Ab < Ba\)    
>     
> 所以可知如果 \(Ab < Ba\)，交換前會小於交換後。
...
>     
> 這是因為我們希望最小化扣款總額，    
> 而扣款總額是由每個工作的完成時間和重要性的乘積之和計算得出的。    
> 因此，我們希望先完成那些重要性高但所需時間短的工作，    
> 以便盡快完成它們並減少扣款。    
>     
> 假設有兩個工作 \(A\) 和 \(B\)，它們的所需時間分別為 \(t_A\) 和 \(t_B\)，    
> 重要性分別為 \(w_A\) 和 \(w_B\)。    
>     
> 如果我們先完成工作 \(A\) 再完成工作 \(B\)，    
> 則扣款總額為 \(w_A \times t_A + w_B \times (t_A + t_B)\)。    
>     
> 如果我們先完成工作 \(B\) 再完成工作 \(A\)，    
> 則扣款總額為 \(w_B \times t_B + w_A \times (t_A + t_B)\)。    
>     
> 兩種情況下的扣款差額為：    
>
>\[
\begin{aligned}
(w_A \times t_A + w_B \times (t_A + t_B)) &- (w_B \times t_B + w_A \times (t_A + t_B)) \\
&= w_A \times t_A + w_B \times t_A - w_A \times t_B \\
&= (w_A / t_A - w_B / t_B) \times t_A \times t_B
\end{aligned}
\]
> 
>     
> 因此，如果 \( \frac{w_A}{t_A} > \frac{w_B}{t_B} \)，    
> 則先完成工作 \(A\) 再完成工作 \(B\) 的扣款總額較小；    
> 反之，如果 \( \frac{w_A}{t_A} < \frac{w_B}{t_B} \)，    
> 則先完成工作 \(B\) 再完成工作 \(A\) 的扣款總額較小。    
>     
> 因此，我們可以根據每個工作的「重要性/所需時間」來排序，    
> 並選擇最高比值的工作來執行。希望這對你有所幫助！
...
> 知道合併過程後要開始討論誰先合併，誰後合併。    
>     
> 我們一樣使用「比較交換前後」來證明，因為 \(B\) 與 \(C\) 交換沒意義，我們交換 \(A\) 與 \(B\)。    
>     
> \(A, B\) 交換前最低成本：   
...
> \[
\Rightarrow A + B + B + C + C < A + A + B + C + C
\]
